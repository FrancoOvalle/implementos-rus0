...
async function CONSULTARUSAV2() {
    let ptt = await cars.find({marca:"SCANIA"},{modeloId:1,carId:1,marca:1,_id:0}).lean()
    let chasisArr = []
    for (let tec of ptt) {
    ...
    const data = await request({
      uri: `https://aftermarket.catalogs-parts.com/cat_scripts/get_subgroup.php?catalog=cv&word=all&client=1&lang=es&model_id=${modelId}&typ_id=${vehicleId}&str_id=${id_tipo}`,
